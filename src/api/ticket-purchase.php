...
switch ($_GET["action"]) {
  ...
  case "get-date-list":
    $statement = mysqli_prepare($conn, "
        ...
        JOIN Theater ON Scheduled_Movie.theater_id = Theater.theater_id
        JOIN Cinema ON Theater.cinema_id = Cinema.cinema_id
        WHERE movie_id = ? AND Cinema.cinema_id = ? AND scheduled_movie_showing_date >= ? 
    ");
  ...
  case "get-time-list":
    $statement = mysqli_prepare($conn, "
        ...
        JOIN Theater ON Scheduled_Movie.theater_id = Theater.theater_id
        JOIN Cinema ON Theater.cinema_id = Cinema.cinema_id
        WHERE movie_id = ? AND Cinema.cinema_id = ? AND scheduled_movie_showing_date >= ? AND scheduled_movie_starting_time >= ?
    ");
